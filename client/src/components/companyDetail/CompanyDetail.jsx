...
import person from '../../assets/womaneating.jpg'
...
const CompanyDetail = () => {
  ...
  const handleContactOwner = async (e) => {
    ...
    emailjs.sendForm("service_pv2p2er", "template_o3xj9u1", formRef.current, 'IdEP1UxwKztZ6UL9_')
      .then((result) => {
        handleCloseForm()
        setSuccess(true)
        setTimeout(() => {
  ...
  const handleDelete = async () => {
    try {
      await request(`/company/${ id}`, 'DELETE', { 'Authorization': `Bearer ${token}` })
      navigate('/')
    } catch (error) {
      console.log(error)
    }
  ...
  const handleBookmark = async () => {
    try {
      await request(`/company/bookmark/${id}`, 'PUT', { Authorization: `Bearer ${token}` })
  ...
  return (
    <div className={classes.container}>
      <h3 style={{ textAlign: 'center', marginBottom: '2.5rem', fontSize: '32px', marginTop: '-2.5rem' }}>Company Details</h3>
      <div className={classes.wrapper}>
        <div className={classes.left}>
          <img src={`http://localhost:5000/images/${companyDetail?.img}`} />
        </div>
        <div className={classes.right}>
          <h3 className={classes.title}>
            Title: {`${companyDetail?.title}`}
            {user?._id === companyDetail?.currentOwner?._id && (
              <div className={classes.controls}>
                <Link to={`/editCompany/${id}`}>Edit</Link>
          ...
          <div className={classes.details}>
            <div className={classes.typeAndContinent}>
              <div>Type: <span>{`${companyDetail?.type}`}</span></div>
              
            </div>
            <div className={classes.priceAndOwner}>
              <span className={classes.price}><span>Price: $ </span>{`${companyDetail?.price}`}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                Owner: {companyDetail?.currentOwner?.profileImg
                  ? (
                    <img src={`http://localhost:5000/images/${companyDetail?.currentOwner?.profileImg}`} className={classes.owner} />
                  ) : (
          ...
          <p className={classes.desc}>
            Desc: <span>{`${companyDetail?.desc}`}</span>
          </p>
          {user?._id != null && (user?._id !== companyDetail?.currentOwner?._id.toString()) &&
        ...
        <div className={classes.contactForm} onClick={handleCloseForm}>
          <div className={classes.contactFormWrapper} onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleContactOwner} ref={formRef}>
              ...
              <input value={companyDetail?.currentOwner?.email} type="email" placeholder='Owner email' name="to_email" />
      ...
      {success && (
        <div className={classes.success}>
          You've successfully contacted the owner of the Restaurant!
        </div>
      )}
      {shortComment && (
        <div>
